...
### Organization
* Use Zurb Foundation for a grid framework.
...
* Prefer `def self.method`, not `def Class.method` when has just one class method.
* Use `class << self` when has more then one class method.
...
* Define one public methods: `perform`.
* Put workers classes in `app/workers`.
